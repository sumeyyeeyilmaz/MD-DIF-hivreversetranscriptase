"""Cross-validate the external set - Null model.

The genotype-only reference: the model sees the 1388-bit isolate mutation
vector and nothing that identifies the inhibitor, so a fixed trained model
returns the same value for every drug tested against one isolate and cannot
rank two inhibitors. That inability is deliberate - it is what makes this the
baseline the fingerprint configurations are measured against.
...
def columns() -> np.ndarray:
    ...
    The null model sees the isolate bits and nothing else, so the 512
    fingerprint columns of the design matrix are withheld. That single
    restriction is the whole difference from the Morgan configurations.
    """
    return np.arange(N_MUTATIONS)
