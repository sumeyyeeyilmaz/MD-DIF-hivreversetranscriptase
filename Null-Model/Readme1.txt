Null-Model: the genotype-only reference. The model sees the 1388-bit isolate
mutation vector and nothing that identifies the inhibitor, so a fixed trained
model returns the same value for every drug tested against one isolate and
cannot rank two inhibitors. That inability is deliberate - it is what makes this
the baseline the Morgan-206 and Morgan-512 configurations are measured against.
...
Final.csv.gz: Includes Stanford data for 10 RTIs (downloaded 22/02/2023) and the ChEMBL-curated external dataset, already encoded. Column LFC is the observed log10 fold change; mut_0001-mut_1388 describe the isolate and morgan_000-morgan_511 the compound. This configuration reads only the mutation columns.
...
morgan_drugs.csv: Representation of inhibitors for the Stanford 10 RTIs. Used by analysis.py to describe the external set, never by the model.
morgan_chembl.csv.gz: Representation of inhibitors for the ChEMBL database. Used by analysis.py to describe the external set, never by the model.
morgan_map.csv: The 206 fingerprint bits that vary across the training panel. Used by the Morgan-206 configuration; this one reads no fingerprint at all.
...
main.py: This code generates predictions by cross-validating the external dataset and saves these predictions to a file. Its columns() function is what withholds the fingerprint from the model.
