"""Cross-validate the external set - Morgan-512.

The full drug-isolate model: the model sees the 1388-bit isolate mutation
vector and the complete 512-bit Morgan fingerprint of the inhibitor, so one
model covers all ten reverse transcriptase inhibitors and can tell two drugs
...
def columns() -> np.ndarray:
    ...
    Morgan-512 sees everything: the isolate bits and the full fingerprint.
    """
    return np.arange(N_MUTATIONS + N_MORGAN)
