Morgan-206: the reduced drug-isolate model. The model sees the 1388-bit isolate
mutation vector and the 206 fingerprint bits that vary across the ten training
...
Final.csv.gz: Includes Stanford data for 10 RTIs (downloaded 22/02/2023) and the ChEMBL-curated external dataset, already encoded. Column LFC is the observed log10 fold change; mut_0001-mut_1388 describe the isolate and morgan_000-morgan_511 the compound. This configuration reads the mutation columns plus the 206 fingerprint columns listed in morgan_map.csv.
...
morgan_map.csv: The 206 fingerprint bits that vary across the training panel. This is what defines the configuration, so a copy is kept in Training as well as in Analysis.
